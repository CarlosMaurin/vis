...
interface AnimatedServiceCardProps extends ServiceCardProps {
  ...
  /**
   * Mobile only — inverts the card background to bg-primary (#316765) so
   * card 2 stands out visually from cards 1 & 3 (bg-[#2E2D3A]).
...
const ServiceCard: React.FC<AnimatedServiceCardProps> = ({
  ...
  // ── Card background ──────────────────────────────────────────────────────
  // Desktop: always bg-[#2E2D3A] (className applied below, no override).
  // Mobile inverted (card 2): bg-primary (#316765 — brand teal).
  // Mobile normal (cards 1 & 3): bg-[#2E2D3A].
  const cardBg = isMobile && mobileInverted ? 'bg-primary' : 'bg-[#2E2D3A]';

  // ── Decorative border gradient ───────────────────────────────────────────
  // For the inverted card we use a lighter conic gradient so the border
  // stays visible against the primary background.
  ...
  // ── "Explore More" button ─────────────────────────────────────────────────
  // On the inverted card the button uses the dark bg to contrast with the
  // teal card surface. On all other cards it stays bg-primary.
  const btnBg = isMobile && mobileInverted
    ? 'bg-[#2E2D3A] hover:bg-accent'
  ...
  return (
    <motion.div
      ...
      className={`group relative w-full ${cardBg} rounded-[2.25rem] overflow-hidden shadow-2xl ${className}`}
    >
      ...
      >
        <div className="h-full" style={{ paddingBottom: FOOTER_SAFE }}>
          <div>
            <div
              className="inline-flex items-center rounded-full bg-white/10 backdrop-blur-md border border-white/15 font-bold uppercase tracking-[0.18em] text-accent"
              style={{
        ...
        >
          <button
            ...
            className={`group/btn relative inline-flex items-center gap-3 ${btnBg} text-white font-bold transition-all duration-300 shadow-lg overflow-hidden whitespace-nowrap`}
            style={{
              borderRadius: 999,
...
const Services: React.FC = () => {
  ...
  // Card 1 → 0.14 (was 0.22 — moved earlier so title isn't alone too long)
  // Card 2 → 0.48 | Card 3 → 0.72 (unchanged)
  ...
  useMotionValueEvent(scrollYProgress, 'change', (latest) => {
    ...
    else if (latest >= 0.14) setMobileCardsEntered(1);
  ...
  // ── Mobile title: auto fade-in via useInView ─────────────────────────────
  // Replaces the old scroll-driven useTransform approach.
  // The title animates automatically as soon as the section enters the viewport
  // — no manual scroll needed — giving it a clean entry on section arrival.
    ...
    <section id="services" className="bg-cream scroll-mt-28 md:scroll-mt-36">
      <style>{`
        .elegant-scroll {
          ...
          /* scroll-behavior: smooth eliminado: puede interferir con gestos nativos en iOS */
        }
        ...
        <div className="sticky top-0 w-full overflow-hidden px-6" style={{ height: '100svh' }}>
          <div className="max-w-7xl mx-auto w-full h-full">
            <div className="h-full flex flex-col justify-center pt-28 md:pt-32 pb-16 md:pb-20">
              ...
              {/* ── Mobile title — auto fade-in via useInView ────────────
                  Fires once as soon as the section enters the viewport.
                  No manual scroll required. Desktop is completely unaffected.
              ─────────────────────────────────────────────────────────── */}
              <div className="md:hidden mb-10" ref={mobileTitleRef}>
                <motion.div
                  initial={{ opacity: 0, y: 22, scale: 0.985 }}
                  animate={mobileTitleInView
              ...
              {/* ── Mobile cards — stacking animation ────────────────────
                  Card 1: threshold 0.14 (earlier than before — reduces the
                           time the title sits alone on the cream background).
                  Card 2: mobileInverted → bg-primary so it visually differs
                           from cards 1 & 3. Button uses dark bg for contrast.
                  Cards 2 & 3 thresholds and all spring physics unchanged.
              ─────────────────────────────────────────────────────────── */}
